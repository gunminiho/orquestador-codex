...
| Non-Git serialization              | `locking.test.ts`, `runtime.test.ts`: atomic acquisition, competitors, owner-only release, conservative restart adoption, cancellation cleanup                                            |
| Non-Git evidence                   | `ownership.test.ts`: snapshot attribution, explicitly distinct from Git; mixed topology tested separately                                                                                 |
| Cancellation                       | `cancellation.test.ts`, `runtime.test.ts`: intent before interrupt, no cleanup on failed interrupt, terminal tombstone, late reports/reviews/completion                                   |
...
| Migration and durability           | `durability.test.ts`: V1 migration/backup, malformed originals preserved, atomic concurrent workflow and orchestrator-state writes                                                        |
