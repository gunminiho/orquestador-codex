...
| Approved delivery and integration  | `approved-delivery.test.ts`, `delivery-recovery.test.ts`: approval remains recoverable through commit, metadata persistence, cleanup, restart, and partial multi-repository finalization; result branches remain reachable and clean originals integrate safely |
...
| Migration and durability           | `durability.test.ts`, `reconciliation-claims.test.ts`: V1 migration/backup, malformed originals preserved, atomic concurrent workflow writes, serialized stale-snapshot state merges, and recoverable reconciliation claims                                     |
...
- Architect approval creates a durable result commit on the orchestrator branch
  during `FINALIZING_DELIVERY`, before its disposable worktree is removed. The
  terminal `APPROVED` state therefore always has durable delivery metadata.
  `npm run workflow:integrate -- --project <id> --workflow <id>` only
  fast-forwards a clean, unchanged original
