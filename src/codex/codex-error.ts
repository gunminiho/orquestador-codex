...
type StructuredError = { message?: string; codexErrorInfo?: CodexErrorInfo | null; data?: { codexErrorInfo?: CodexErrorInfo | null }; error?: { data?: { codexErrorInfo?: CodexErrorInfo | null } } };

export function classifyCodexError(error: unknown): CodexErrorClassification {
  const structured = (error ?? {}) as StructuredError;
  const info = structured.codexErrorInfo ?? structured.data?.codexErrorInfo ?? structured.error?.data?.codexErrorInfo;
  ...
  if (info === "serverOverloaded" || info === "internalServerError") return { kind: "TRANSIENT_SERVER", message, retryable: true };
  if (typeof info === "object" && info !== null && ("httpConnectionFailed" in info || "responseStreamConnectionFailed" in info || "responseStreamDisconnected" in info || "responseTooManyFailedAttempts" in info)) return { kind: "TRANSIENT_SERVER", message, retryable: true };
