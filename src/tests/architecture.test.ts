...
test("runner advances all executable states", async () => { const root=await fixture(); const store=new WorkflowStore(root); const engine=new WorkflowEngine(store); let calls=0; const architect={sendStructured:async()=>({data:calls++===0?assignment:{type:"REVIEW_RESULT",taskId:"t",reviewedAgent:"frontend",decision:"APPROVED",summary:"ok",findings:[],requiredChanges:[],validationRequired:[]}})}; const router={routeTask:async()=>report}; const runner=new WorkflowRunner(engine,store,architect as never,router as never,new OwnershipVerifier(new TopologyService(topology(root)))); const result=await runner.runUntilPauseOrTerminal(await engine.create("project","x","loop")); assert.equal(result.state,"APPROVED"); });
test("rate limit remains paused and ids are safe", async()=>{const root=await fixture();const e=new WorkflowEngine(new WorkflowStore(root));let w=await e.create("project","x","safe");w=await e.transition(w,"PLANNING","x");w=await e.pauseForError(w,new Error("429 rate limit"));assert.equal(w.state,"PAUSED_RATE_LIMIT");await assert.rejects(()=>e.create("../x","x","safe"));});
