...
test("normalizes official resetsAt Unix seconds", async () => {
  ...
  const now = 1_700_000_000_000; const sleeps: number[] = []; let calls = 0;
  const limited = { rateLimits: { rateLimitReachedType: "primary", primary: { resetsAt: 1_700_000_120, usedPercent: 100, windowDurationMins: 1 }, secondary: null }, rateLimitsByLimitId: null };
  const available = { rateLimits: { rateLimitReachedType: null, primary: null, secondary: null }, rateLimitsByLimitId: null };
  const scheduler = new RateLimitScheduler(engine, { getAccountRateLimits: async () => (++calls === 1 ? limited : available) } as never, async (ms) => { sleeps.push(ms); }, () => now);
  const result = await scheduler.waitForAvailability(workflow);
  assert.equal(result.state, "PLANNING");
  assert.equal(sleeps[0], 120_000);
});
