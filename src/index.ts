...
const store = new WorkflowStore(root); const engine = new WorkflowEngine(store); const ownership = new OwnershipVerifier(topology); const router = new TeamRouter(backend, frontend); const runner = new WorkflowRunner(engine, architect, router, ownership); const recovery = new WorkflowRecovery(engine, client);
