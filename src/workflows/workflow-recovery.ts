...
export class WorkflowRecovery {
  ...
  async discover(projectId: string): Promise<Workflow[]> {
    return this.engine.recover(projectId);
  }
  async resumeRateLimited(workflow: Workflow): Promise<Workflow> {
    ...
    ].some((snapshot) => snapshot?.rateLimitReachedType !== null);
    ...
    return this.engine.transition(
      { ...workflow, rateLimit: null },
      checkpoint,
      "Rate-limit snapshot permits continuation; action remains checkpointed",
    );
  }
}
