...
export class RateLimitScheduler {
  constructor(private readonly engine: WorkflowEngine, private readonly client: RateLimitClient, private readonly sleep: Sleeper = (milliseconds) => new Promise((resolve) => setTimeout(resolve, milliseconds)), private readonly now = () => Date.now()) {}
  async waitForAvailability(workflow: Workflow): Promise<Workflow> {
    let current = workflow;
    while (current.state === "PAUSED_RATE_LIMIT") {
      const persistedCheck = current.rateLimit?.nextCheckAt ? Date.parse(current.rateLimit.nextCheckAt) : 0;
      if (persistedCheck > this.now()) await this.sleep(persistedCheck - this.now());
